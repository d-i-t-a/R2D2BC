...
export default class LineFocusModule implements ReaderModule {
  ...
  disableLineFocus() {
    ...
    this.highlighter.destroyHighlights(HighlightType.Lines, 0);
  }
  ...
  lineFocus() {
    ...
    const highlightsContainer = this.highlighter.ensureHighlightsContainer(
      ID_LINEFOCUS_CONTAINER,
      0
    );
    ...
    for (const clientRect of newGroupedLines) {
      ...
      const documant = (this.delegate.iframes[0].contentWindow as any).document;
      ...
      highlightsContainer.append(highlightArea);
    }
    setTimeout(() => {
      this.currentLine();
    }, 100);
