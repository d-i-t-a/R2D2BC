...
//
// AudioWorklet processor for pitch preservation via spectral pitch shifting.
//
// Adopted verbatim from Readium ts-toolkit
// (navigator/src/audio/engine/PreservePitchProcessor.js), which is licensed
// under BSD 3-Clause:
//   Copyright (c) 2022, Readium Foundation. All rights reserved.
//   https://github.com/readium/ts-toolkit/blob/main/LICENSE
//
...
//   - Overlap-add (OLA) phase vocoder with iterative in-place
//     Cooley-Tukey FFT/IFFT.
//   - All intermediate buffers are pre-allocated in the constructor; the
//     hot path (process → _processBuffer → _fft) is allocation-free and
//     GC-safe.
//   - Output ring buffer decouples OLA processing (fires every hopSize
//     input samples) from the Web Audio render quantum (128 frames), so
//     process() always fills outputChannel completely.
//
// Worklet name (registerProcessor): "preserve-pitch-processor". Mirrored
// in PreservePitchProcessor.ts as PRESERVE_PITCH_PROCESSOR_NAME so the
// registration site and the AudioWorkletNode constructor share one
// constant.
