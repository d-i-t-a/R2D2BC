...
export default class ContentProtectionModule implements ReaderModule {
  ...
  protected async start(): Promise<void> {
    ...
    if (oc(this.config).enableObfuscation(false)) {
  ...
  async stop() {
    ...
    if (oc(this.config).disableKeys(false)) {
      removeEventListenerOptional(
      ...
      if (this.delegate.iframe2) {
        removeEventListenerOptional(
          this.delegate.iframe2.contentDocument,
          "keydown",
          this.disableSave
        );
        removeEventListenerOptional(
          this.delegate.iframe2.contentWindow,
          "keydown",
          this.disableSave
        );
      }
    ...
    if (oc(this.config).disableCopy(false)) {
      removeEventListenerOptional(
        this.delegate.mainElement,
        "copy",
        this.preventCopy
    ...
    if (oc(this.config).disablePrint(false)) {
      removeEventListenerOptional(
        this.delegate.mainElement,
        "beforeprint",
    ...
    if (oc(this.config).disableContextMenu(false)) {
      removeEventListenerOptional(
        this.delegate.mainElement,
        "contextmenu",
        this.disableContext
    ...
    if (oc(this.config).hideTargetUrl(false)) {
      this.hideTargetUrls(false);
    }
    if (oc(this.config).disableDrag(false)) {
  ...
  private setupEvents(): void {
    var self = this;
    if (oc(this.config).detectInspect(false)) {
      var checkStatus = "off";
      ...
      requestAnimationFrame(function check() {
        ...
        console.log(div);
        if (checkStatus === "on") {
          if (oc(self.config).clearOnInspect(false)) {
            console.clear();
          ...
          if (
            oc(self.config).api &&
            oc(self.config).api.inspectDetected(false)
          ) {
    ...
    if (oc(this.config).disableKeys(false)) {
      ...
      addEventListenerOptional(
        this.delegate.iframe.ownerDocument,
        "keydown",
        this.disableSave
      );
      addEventListenerOptional(
      ...
      addEventListenerOptional(
        this.delegate.iframe.contentWindow.document,
        "keydown",
        this.disableSave
      );
      if (this.delegate.iframe2) {
        addEventListenerOptional(
          this.delegate.iframe2.ownerDocument,
          "keydown",
          this.disableSave
        );
        addEventListenerOptional(
        ...
        addEventListenerOptional(
          this.delegate.iframe2.contentWindow.document,
          "keydown",
          this.disableSave
        );
      }
  ...
  hideTargetUrls(activate) {
    ...
    const aElements = this.delegate.iframe.contentDocument.querySelectorAll(
      "a"
    );
    ...
    if (this.delegate.iframe2) {
      const aElements = this.delegate.iframe2.contentDocument.querySelectorAll(
        "a"
      );
  ...
  preventDrag(activate) {
    ...
    const bodyStyle =
      this.delegate.iframe.contentDocument.body.getAttribute("style") || "";

    if (activate) {
      this.delegate.iframe.contentDocument.body.addEventListener(
        "dragstart",
        onDragstart
      );
      this.delegate.iframe.contentDocument.body.setAttribute(
        "style",
        bodyStyle + dragStyle
      );
    } else {
    ...
    if (this.delegate.iframe2) {
      const bodyStyle =
        this.delegate.iframe2.contentDocument.body.getAttribute("style") || "";

      if (activate) {
        this.delegate.iframe2.contentDocument.body.addEventListener(
          "dragstart",
          onDragstart
        );
        this.delegate.iframe2.contentDocument.body.setAttribute(
          "style",
          bodyStyle + dragStyle
        );
      } else {
        this.delegate.iframe2.contentDocument.body.removeEventListener(
          "dragstart",
          onDragstart
        );
        this.delegate.iframe2.contentDocument.body.setAttribute(
