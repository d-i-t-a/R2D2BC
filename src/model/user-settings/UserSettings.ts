  ...
  public static async create(config: UserSettingsConfig): Promise<any> {
    ...
    if (config.initialUserSettings) {
      ...
      if (initialUserSettings.verticalScroll) {
        settings.verticalScroll = UserSettings.scrollValues.findIndex(
          (el: any) => el === initialUserSettings.verticalScroll
        );
    ...
    setTimeout(async () => {
      if (userSettings.verticalScroll != undefined) {
        ...
        if (
          ...
          userSettings.verticalScroll == "readium-scroll-on" ||
          userSettings.verticalScroll == true
        ) {
          v = UserSettings.scrollValues[0];
        } else if (
          userSettings.verticalScroll == "paginated" ||
          userSettings.verticalScroll == "readium-scroll-off" ||
