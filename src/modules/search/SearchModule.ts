...
export default class SearchModule implements ReaderModule {
    ...
    async stop() {
        if (IS_DEV) { console.log("Search module stop") }
    ...
    protected async start(): Promise<void> {
        ...
        if (this.headerMenu) {
            this.searchInput = HTMLUtilities.findElement(this.headerMenu, "#searchInput") as HTMLInputElement;
            addEventListenerOptional(this.searchInput, 'keypress', this.handleSearch.bind(this));

            this.searchGo = HTMLUtilities.findElement(this.headerMenu, "#searchGo") as HTMLElement;
            addEventListenerOptional(this.searchGo, 'click', this.handleSearch.bind(this));

            // CONTROL
            var menuSearch = HTMLUtilities.findElement(this.headerMenu, "#menu-button-search") as HTMLLinkElement;
        ...
        async function goToResultPage(page: number) {
            ...
            if (paginated.total === 0) {
                ...
                for (let index = 0; index < paginated.data.length; index++) {
                    ...
                    linkElement.innerHTML = "..." + element.textBefore + "<mark>" + element.textMatch + "</mark>" + element.textAfter + "..." //   element.chapter_highlight
                ...
                previousResultPage.innerHTML = "<a href=\"#!\">" + "left" /*IconsAllvit.arrowLeft*/ + "</a>"
                if (paginated.pre_page != null) {
                    previousResultPage.className = "waves-effect";
                    addEventListenerOptional(previousResultPage, 'click', (event: MouseEvent) => {
                        event.preventDefault();
                        event.stopPropagation();
                ...
                nextResultPage.innerHTML = "<a href=\"#!\">" + "right" /*IconsAllvit.arrowRight*/ + "</a>"
                if (paginated.next_page != null) {
                    nextResultPage.className = "waves-effect";
                    addEventListenerOptional(nextResultPage, 'click', (event: MouseEvent) => {
                        event.preventDefault();
                        event.stopPropagation();
            ...
            if (paginated.total === 0) {
                ...
                paginatedGrouped.forEach(chapter => {
                    ...
                    chapter.forEach(searchItem => {
                        ...
                        linkElement.innerHTML = "..." + searchItem.textBefore + "<mark>" + searchItem.textMatch + "</mark>" + searchItem.textAfter + "..." //searchItem.chapter_highlight
                        addEventListenerOptional(linkElement, 'click', (event: MouseEvent) => {
                            event.preventDefault();
                            event.stopPropagation();
