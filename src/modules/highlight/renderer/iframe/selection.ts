...
export function getCurrentSelectionInfo(
    ...
    if (selection.isCollapsed) {
        if (IS_DEV) {console.log("^^^ SELECTION COLLAPSED.");}
        return undefined;
    }
    ...
    if (cleanText.length === 0) {
        if (IS_DEV) {console.log("^^^ SELECTION TEXT EMPTY.");}
        return undefined;
    }
    ...
    if (!r || r.collapsed) {
        if (IS_DEV) {console.log("$$$$$$$$$$$$$$$$$ CANNOT GET NON-COLLAPSED SELECTION RANGE?!");}
        return undefined;
    }
    ...
    if (!rangeInfo) {
        if (IS_DEV) {console.log("^^^ SELECTION RANGE INFO FAIL?!");}
        return undefined;
    }
...
export function createOrderedRange(startNode: Node, startOffset: number, endNode: Node, endOffset: number):
    ...
    if (!range.collapsed) {
        if (IS_DEV) {console.log(">>> createOrderedRange RANGE OK");}
        return range;
    }

    if (IS_DEV) {console.log(">>> createOrderedRange COLLAPSED ... RANGE REVERSE?");}
    const rangeReverse = new Range(); // document.createRange()
    rangeReverse.setStart(endNode, endOffset);
    rangeReverse.setEnd(startNode, startOffset);
    if (!rangeReverse.collapsed) {
        if (IS_DEV) {console.log(">>> createOrderedRange RANGE REVERSE OK.");}
        return range;
    }

    if (IS_DEV) {console.log(">>> createOrderedRange RANGE REVERSE ALSO COLLAPSED?!");}
    return undefined;
}
...
export function convertRange(
    ...
    if (!commonElementAncestor) {
        if (IS_DEV) {console.log("^^^ NO RANGE COMMON ANCESTOR?!");}
        return undefined;
    }
...
export function convertRangeInfo(documant: Document, rangeInfo: IRangeInfo):
    ...
    if (!startElement) {
        if (IS_DEV) {console.log("^^^ convertRangeInfo NO START ELEMENT CSS SELECTOR?!");}
        return undefined;
    }
    ...
    if (rangeInfo.startContainerChildTextNodeIndex >= 0) {
        if (rangeInfo.startContainerChildTextNodeIndex >= startElement.childNodes.length) {
            if (IS_DEV) {console.log("^^^ convertRangeInfo rangeInfo.startContainerChildTextNodeIndex >= startElement.childNodes.length?!");}
            return undefined;
        }
        startContainer = startElement.childNodes[rangeInfo.startContainerChildTextNodeIndex];
        if (startContainer.nodeType !== Node.TEXT_NODE) {
            if (IS_DEV) {console.log("^^^ convertRangeInfo startContainer.nodeType !== Node.TEXT_NODE?!");}
            return undefined;
        }
    }
    const endElement = documant.querySelector(rangeInfo.endContainerElementCssSelector);
    if (!endElement) {
        if (IS_DEV) {console.log("^^^ convertRangeInfo NO END ELEMENT CSS SELECTOR?!");}
        return undefined;
    }
    let endContainer: Node = endElement;
    if (rangeInfo.endContainerChildTextNodeIndex >= 0) {
        if (rangeInfo.endContainerChildTextNodeIndex >= endElement.childNodes.length) {
            if (IS_DEV) {console.log("^^^ convertRangeInfo rangeInfo.endContainerChildTextNodeIndex >= endElement.childNodes.length?!");}
            return undefined;
        }
        endContainer = endElement.childNodes[rangeInfo.endContainerChildTextNodeIndex];
        if (endContainer.nodeType !== Node.TEXT_NODE) {
            if (IS_DEV) {console.log("^^^ convertRangeInfo endContainer.nodeType !== Node.TEXT_NODE?!");}
            return undefined;
        }
    }
